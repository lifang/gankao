...
class User::HomesController < ApplicationController
  ...
  def show
    ...
    @all_examinations = Examination.find_by_sql("select count(types) sums, types from examinations
      where is_published = #{Examination::IS_PUBLISHED[:ALREADY]} and category_id=#{params[:id].to_i} and types != #{Examination::TYPES[:SIMULATION]} group by types")
    @user_exams = Examination.find_by_sql("select count(types) sums,e.types from examinations e
      ...
      where e.types != #{Examination::TYPES[:SIMULATION]} and category_id=#{params[:id].to_i} and u.is_submited = #{ExamUser::IS_SUBMITED[:YES]}
