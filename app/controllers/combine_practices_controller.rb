...
class CombinePracticesController < ApplicationController
  ...
  def start
    user_examination=Examination.find_by_sql("select ex.id,eu.is_submited from examinations ex left join exam_users eu on ex.id=eu.examination_id where ex.types=#{Examination::TYPES[:PRACTICE]} and eu.user_id=#{cookies[:user_id]} and eu.is_submited=1")[0]
    if user_examination
      redirect_to "/user/combine_practices/#{user_examination.id}/start",:target=>"_blank"
      return 0
    end  #优先进入已经打开的综合测试
    can_practies = true
