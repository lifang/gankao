...
class PagesController < ApplicationController
  ...
  def qq_index
    ...
    ##=======
    params="oauth_client_ip=116.255.140.79&oauth_consumer_key=#{app_id}&oauth_nonce=#{timestamp}&oauth_signature_method=HMAC-SHA1&oauth_timestamp=#{timestamp}&oauth_token=#{oauth_token}&oauth_vericode=#{oauth_vericode}&oauth_version=1.0"
    puts session[:secret]
  ...
  def add_user
    timestamp=(Time.new.to_i).to_s
    begin  
      user_params="format=json&oauth_client_ip=116.255.140.79&oauth_consumer_key=#{app_id}&oauth_nonce=#{timestamp}&oauth_signature_method=HMAC-SHA1&oauth_timestamp=#{timestamp}&oauth_token=#{session[:qqtoken]}&oauth_version=1.0&open_id=#{session[:qqopen_id]}"
