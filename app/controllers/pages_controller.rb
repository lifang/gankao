...
class PagesController < ApplicationController
  ...
  def login_from_qq
    ...
    url_params = "#{COMSUMER_KEY}&oauth_nonce=#{timestamp}&#{SIGNATRUE_METHOD}&oauth_timestamp=#{timestamp}&#{VESION}"
    url="#{REQUEST_URL}?#{url_params}&oauth_signature=#{signature_params(COMSUMER_SECRECT,url_params,REQUEST_URL,"GET","")}"
  ...
  def qq_index
    ...
    begin
      ...
      user_params="#{COMSUMER_KEY}&oauth_nonce=#{timestamp}&#{SIGNATRUE_METHOD}&oauth_timestamp=#{timestamp}&oauth_token=#{session[:qqtoken]}&#{VESION}&openid=#{session[:qqopen_id]}"
      user_url="#{GRAPY_URL}?#{user_params}&oauth_signature=#{signature_params(COMSUMER_SECRECT,user_params,GRAPY_URL,"GET",session[:qqsecret])}"
